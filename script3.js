...
const playerWins = function () {
  //When player wins, this pops up
  show(correct);
  show(overlay);
};

const playerFails = function () {
  //When player fails, this pops up
  show(gameOver);
  show(overlay);
};
function showOptions() {
  //Shows the options again after being used
...
const newGame = function () {
  //Setting the interface for a new game
  hide(gameOver);
  hide(correct);
  hide(overlay);
  hide(title);
...
nextButton.addEventListener("click", function () {
  ...
  document.getElementById(
    "wintext"
  ).textContent = `Correct!! You got that right!!`;
...
answerYes.addEventListener("click", function () {
  //yes  button
  restart();
  let takeHome = document.querySelector(
    `.money--${currentQuestion}`
  ).textContent;
  hide(container);
...
function loadQuestion(questionIndex) {
  //Loads question at current index
  ...
  fiftyFifty.addEventListener("click", function () {
    ...
    fiftyFifty.style.backgroundPosition = "center center";
    fiftyFifty.style.backgroundRepeat = "no-repeat";

    fiftyFifty.disabled = true; //disables the button after use
  });
  askAudience.addEventListener("click", function () {
    //ask the audience life-line
    show(overlay);
    show(audiencePopUp);
    ...
    overlay.addEventListener("click", function () {
      ...
      askAudience.style.backgroundPosition = "center center";
      askAudience.style.backgroundRepeat = "no-repeat";
      hide(help1);
  ...
  phoneBtn.addEventListener("click", function () {
    ...
    const randomLetterGenerator = Math.trunc(
    ...
    overlay.addEventListener("click", function () {
      ...
      phoneBtn.style.backgroundPosition = "center center";
      phoneBtn.style.backgroundRepeat = "no-repeat";
...
answerModalArray.forEach(function (val) {
  ...
  val.addEventListener("click", function () {
    if (val.innerText === questionsToDisplay[currentQuestion].answers[4]) {
      ...
      if (score == 4 || score == 7) {
        //conditions for guaranteed money
        document.querySelector(`.money--${score}`).style.backgroundColor =
          "green";
        document.querySelector(`.money--${score - 3}`).style.backgroundColor =
          "white";
        let guarantee = document.querySelector(`.money--${score}`).innerText;
        document.getElementById(
          "wintext"
        ).textContent = `You have ${guarantee} guaranteed`;
      }
      playerWins();
      setInterval(() => {
      ...
      document
        .querySelector(`.money--${score - 1}`)
        .classList.remove("money-hover");
    } else {
      ...
      if (score < 4) {
        ...
        document.getElementById("score-value").textContent =
          document.querySelector(`.money--${score}`).textContent;
      }
      playerFails();
    }
  });
